...
import com.example.urbanin.account.AccountFragmentDirections
import com.example.urbanin.data.LoginPreferenceManager
import com.example.urbanin.databinding.FragmentAccountBinding
import com.example.urbanin.databinding.RoommateAccountHomeBinding
...
class AccountHome : Fragment() {
    private lateinit var binding: RoommateAccountHomeBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = RoommateAccountHomeBinding.inflate(layoutInflater)
        ...
        with(binding) {
            ...
            Settings.setOnClickListener {
                val action = AccountHomeDirections.actionAccountHomeToAccountSettings()
                findNavController().navigate(action)
            }
            Profile.setOnClickListener {
                val action = AccountHomeDirections.actionAccountHomeToAccountProfile()
                findNavController().navigate(action)
            }
            ChangePassword.setOnClickListener {
                val action = AccountHomeDirections.actionAccountHomeToAccountChangePwd()
                findNavController().navigate(action)
            }
        }
    ...
    private val pickFromGallery =
        registerForActivityResult(ActivityResultContracts.PickVisualMedia()) { uri ->
            ...
            if (uri != null) {
                Log.d(MainActivity.TAG, "Profile Image selected: $uri")
                // add profile image to view
                binding.ProfileImage.setImageURI(uri)
                uploadToFirebaseStorage(uri)
            } else {
                Log.d(MainActivity.TAG, "No profile image selected")
            }
        }
    ...
    private fun uploadToFirebaseStorage(imageUri: Uri) {
        ...
        fileReference.putFile(imageUri)
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    fileReference.downloadUrl.addOnSuccessListener { uri ->
                        Log.d(MainActivity.TAG, "Profile Image File added: $uri")
                        ...
                        db.collection("Users")
                            ...
                            .addOnSuccessListener {
                                Log.d(MainActivity.TAG, "Saved profile image to Users collection")
                            }.addOnFailureListener { exception ->
                                Log.e(
                                    MainActivity.TAG,
                                    "Couldn't save image to Users collection- $exception"
                                )
                            }
                    }
                } else {
            ...
            .addOnFailureListener { exception ->
                ...
                Log.e(MainActivity.TAG, "Storage upload error: " + exception.message.toString())
            }
    }
}